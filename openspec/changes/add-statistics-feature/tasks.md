...
- [ ] 4.12 添加下拉刷新功能
- [ ] 4.13 适配不同屏幕尺寸（iPhone SE / Pro Max / iPad）
...
- [ ] 6.1 为 HabitStatistics 计算属性添加 `lazy` 关键字
- [ ] 6.2 测试大数据量场景（100 习惯 + 10000 打卡记录）
- [ ] 6.3 如果性能不佳，使用 `Task` 在后台线程计算
- [ ] 6.4 添加计算耗时监控（Instruments Time Profiler）
- [ ] 6.5 确保统计页面打开耗时 < 100ms

## 7. 可访问性和本地化

- [ ] 7.1 为所有统计卡片添加 VoiceOver 标签
- [ ] 7.2 为趋势图表添加 `.accessibilityLabel()` 描述
- [ ] 7.3 确保所有文本支持 Dynamic Type
- [ ] 7.4 测试 VoiceOver 导航流程
- [ ] 7.5 检查所有中文文案是否清晰准确

## 8. 测试和验证

- [ ] 8.1 手动测试：创建 5 个习惯，添加不同的打卡记录
- [ ] 8.2 验证统计数据计算准确性（对比手工计算）
- [ ] 8.3 测试时间范围切换（全部/本月/本周）
- [ ] 8.4 测试边界情况：空数据、单个习惯、大量数据
- [ ] 8.5 测试连续天数计算准确性（跨月、跨年场景）
- [ ] 8.6 测试删除习惯后统计数据自动更新
- [ ] 8.7 测试新打卡后统计数据自动刷新
- [ ] 8.8 在不同设备上测试布局（iPhone SE, iPhone 15 Pro, iPad）
- [ ] 8.9 测试横屏和竖屏模式
- [ ] 8.10 运行单元测试套件，确保所有测试通过

## 9. 代码质量和文档

- [ ] 9.1 添加代码注释（复杂算法和业务逻辑）
- [ ] 9.2 确保代码符合项目 SOLID 原则
- [ ] 9.3 运行 SwiftLint 检查代码风格
- [ ] 9.4 清理调试代码和 print 语句
- [ ] 9.5 更新 PROJECT_OVERVIEW.md 文档，添加统计功能说明
- [ ] 9.6 更新 README.md，添加统计功能截图
...
- [x] 10.5 在真机上完整测试统计功能
- [ ] 10.6 截取统计页面截图，保存到 `Docs/habit_screenshot/`
- [ ] 10.7 提交代码到 Git 仓库
